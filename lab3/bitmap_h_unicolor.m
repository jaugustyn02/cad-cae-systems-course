...
function bitmap_h_unicolor(filename,elementsx,elementsy,maxerror,max_refinement_level,color_edges_black, color)
if_then_else = @(varargin) varargin{end - varargin{1}};
colors = {'r', 'g', 'b'};

% validate color
if (~ismember(colors, color))
    disp("Provided invalid color!")
    disp("Valid colors: red='r', green='g' or blue='b'.")
    return;
end
...
RR = if_then_else(color=='r', XX(:,:,1), zeros(size(XX(:,:,1))));
GG = if_then_else(color=='g', XX(:,:,2), zeros(size(XX(:,:,2))));
BB = if_then_else(color=='b', XX(:,:,3), zeros(size(XX(:,:,3))));
...
% if any of the errors is higher than our maximum -> break element and repeat entire loop
  ...
  % display image
  type = if_then_else(color_edges_black, 'Grid', 'Approx');
  fig_name = sprintf("%s - Iteration: %d/%d, Img: '%s', Color: '%s', Max error: %g", ...
      type, refinemenet_level, max_refinement_level, filename, color, maxerror);
  figure('Name', fig_name, 'NumberTitle', 'off');
  imshow(RGB);
  axis off;
  img_path = sprintf("%s/%s_%d.jpg", color, type, refinemenet_level);
  %saveas(gcf, img_path);
  exportgraphics(gca, img_path, 'Resolution', 300);
  RR = RR_CP;
  GG = GG_CP;
  BB = BB_CP;
end
