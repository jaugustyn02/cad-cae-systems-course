...
% if any of the errors is higher than our maximum -> break element and repeat entire loop
  ...
  fig_name = sprintf("%s - Iteration: %d/%d, Img: '%s', Max error: %g", ...
      type, refinemenet_level, max_refinement_level, filename, maxerror);
  figure('Name', fig_name, 'NumberTitle', 'off');
  imshow(RGB);
  
  img_path = sprintf("rgb/%s_%d.jpg", type, refinemenet_level);
  %saveas(gcf, img_path);
