function heat_time_matlab(imgFile)
    % Input data
    knot = simple_knot(28, 2);      % knot vector (16,2)
    ...
    bitmap = imread(imgFile);
    bitmap = imresize(bitmap, [nx-8, ny-8]);
    bitmap = double(bitmap) / (256);
    
    u = zeros(nx, ny, size(bitmap, 3));
    u(5:nx-4, 5:ny-4, :) = bitmap;
    ...
    save_plot(u, 0, bx, by, outputFolder);
    ...
    for m = 1:K
      ...
      % Plot the solution
      %if mod(m,10)==0
        save_plot(u, m, bx, by, outputFolder);
      %end
    end
end
...
function save_plot(u, iter, bx, by, outputFolder)
  N = 50;
  ...
  saveas(h, sprintf('%s/out_%d.png', outputFolder, iter));
  close(h);
end
